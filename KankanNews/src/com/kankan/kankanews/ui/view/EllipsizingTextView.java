...
public class EllipsizingTextView extends MyTextView {
	...
	private void resetText() {
		...
		if (maxLines != -1) {
			...
			if (layout.getLineCount() > maxLines) {
				...
				while (createWorkingLayout(workingText + ELLIPSIS)
					...
					// break;
					// }
					workingText = workingText.substring(0, workingText.length()
							- ELLIPSIS.length());
					int i = 0;
				}
				ellipsized = true;
		...
		if (ellipsized != isEllipsized) {
			...
			for (EllipsizeListener listener : ellipsizeListeners) {
				listener.ellipsizeStateChanged(ellipsized, this);
			}
		}
	}
